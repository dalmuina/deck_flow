...
class TimerViewModelTest {
    ...
    fun `when reset then persists cardId`() = runTest {
        every { robot.observeTimerStateUseCase() } returns flowOf(DFResult.Success(PersistedTimerState()))
    ...
    @Test
    fun `when restoring a running persisted state then timer state reflects it running`() = runTest {
